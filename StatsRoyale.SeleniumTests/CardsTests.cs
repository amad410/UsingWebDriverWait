...
namespace StatsRoyale.SeleniumTests
{
    ...
    public class CardsTests : BaseTest
    {
        ...
        public void AssertDifferentCardStatsOnDetailsPage(string cardName)
        {
            ...
            WebDriverWait wait = new WebDriverWait(Driver.Current, TimeSpan.FromSeconds(20));
            wait.PollingInterval = TimeSpan.FromMilliseconds(500);
            wait.IgnoreExceptionTypes(typeof(StaleElementReferenceException), typeof(NoSuchElementException));
            if (wait.Until(drv => drv.FindElement(By.XPath("//div[@class='ui__headerMedium'][contains(text(),'Cards')]")).Displayed) == true)
            {
                Pages.Cards.GetCardByCardName(cardName).Click();
                if (wait.Until(drv => drv.FindElement(By.XPath("//div[text()='Statistics']")).Displayed) == true)
                {
